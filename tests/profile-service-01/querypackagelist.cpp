...
int main(int argc, char **argv)
{
    ...
    ProfileServiceHandler PSHandler(std::string(argv[1]), "querypackagelist/1.2.3");
    ...
    // check for content
    assert(packageList->getName() == "name");
    assert(packageList->getDescription() == "description");
    assert(packageList->getVersion() == "version");
    assert(packageList->getPackagesSize() == 3);
